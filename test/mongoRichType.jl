...
mcoll = Mongo.MongoCollection(mongoClient, "CloudGraphs", "bindata");
testKey = BSONOID("58e5f5940214580915172ae2");
results = first(find(mcoll, ("_id" => eq(testKey))));
@show dict(results)
@show deepcopy(dict(results))
@show dict(results["val"])
